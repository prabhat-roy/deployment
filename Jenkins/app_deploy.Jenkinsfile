...
ipeline {
    ...
    stages {
        ...
        stage('Load Environment Variables') {
            steps {
                script {
                    echo "📦 Copying Jenkins.env from 'tools' pipeline..."
        ...
        stage('ESLint Scan') {
            ...
            stage('OWASP Dependency-Check') {
                steps {
                    script {
                        def owasp = load 'groovy/owasp.groovy'
            ...
            stage('CodeQL Scan') {
                steps {
                    script {
                        // Load the CodeQL scanning script
                        def codeqlScan = load 'groovy/codeql.groovy'
                        
                        // Run the CodeQL scan and archive the reports
                ...
                stage('Docker Scout Scan') {
                    steps {
                        script {
                            // Load the groovy script to manage Docker Scout scan logic
                            def dockerScoutScan = load 'groovy/docker_scout.groovy'
                            
                            // Run the scan and archive the reports
            ...
            stage('Cosign Scan') {
                steps {
                    script {
                        // Load the groovy script to manage the Cosign scan logic
                        def cosignScan = load 'groovy/cosign.groovy'
                        
                        // Run the signing and verification steps
            ...
            stage('Update Kubeconfig') {
                ...
                steps {
                    script {
                        def kubeconfigUpdate = load('groovy/kubeconfig.groovy')
