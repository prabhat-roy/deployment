def pushToCloudRegistry = {
    ...
    def dockerServices = env.DOCKER_SERVICES?.split(',')?.findAll { it?.trim() } ?: []
    ...
    switch (cloudEnv) {
        case 'aws':
            ...
            if (!awsAccountId) error "[ERROR] AWS_ACCOUNT_ID not defined"
            ...
            dockerServices.each { svc ->
                def (image, tag) = svc.tokenize(':') + [null]
                tag = tag ?: 'latest'
                def repoName = image
